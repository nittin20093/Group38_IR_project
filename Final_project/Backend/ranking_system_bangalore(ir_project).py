# -*- coding: utf-8 -*-
"""Ranking system_Bangalore(IR Project)

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1-ExAi8GUbYih6KTv6MuSDlWK6H6sJ1ss
"""
...
df=pd.read_csv("/content/drive/MyDrive/IR_Project/banglore_hotels_details_final.csv")
...
for ind in df.index:
  x=df['Score'][ind]
  if '560001' in df['Location'][ind]:
    x=x*2
  if '560025' in df['Location'][ind]:
    x=x*1.9
  if '560070' in df['Location'][ind]:
    x=x*1.8
  if '560078' in df['Location'][ind]:
    x=x*1.7
  if '560066' in df['Location'][ind]:
    x=x*1.6
  if '560085' in df['Location'][ind]:
    x=x*1.5
  if '560037' in df['Location'][ind]:
    x=x*1.4
  if '560064' in df['Location'][ind]:
    x=x*1.3
  if '560092' in df['Location'][ind]:
    x=x*1.2
  if '560078' in df['Location'][ind]:
    x=x*1.1
  score.append(x)
...
Sith.to_csv('TouristRankedHotelsBangalore.csv', sep='\t', index=False,header=True)
